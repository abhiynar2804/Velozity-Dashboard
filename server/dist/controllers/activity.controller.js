...
class ActivityController {
    async listActivities(req, res, next) {
        try {
            ...
            res
                .status(200)
                .json(ApiResponse_1.ApiResponse.success("Activities fetched successfully", activities));
        }
        catch (error) {
    ...
    async getNotifications(req, res, next) {
        try {
            ...
            res
                .status(200)
                .json(ApiResponse_1.ApiResponse.success("Notifications fetched successfully", notifications));
        }
        catch (error) {
            if (error instanceof auth_service_1.AppError) {
                res
    ...
    async markNotificationRead(req, res, next) {
        try {
            ...
            res
                .status(200)
                .json(ApiResponse_1.ApiResponse.success("Notification marked as read", notification));
        }
        catch (error) {
            if (error instanceof auth_service_1.AppError) {
                res
    ...
    async markAllNotificationsRead(req, res, next) {
        try {
            const result = await activity_service_1.activityService.markAllNotificationsRead(req.user);
            res
                .status(200)
                .json(ApiResponse_1.ApiResponse.success("All notifications marked as read", result));
        }
        catch (error) {
            if (error instanceof auth_service_1.AppError) {
                res
