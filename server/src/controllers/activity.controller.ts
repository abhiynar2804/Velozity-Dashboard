...
export class ActivityController {
  async listActivities(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const activities = await activityService.listActivities(req.user!, projectId);
      res.status(200).json(ApiResponse.success('Activities fetched successfully', activities));
    } catch (error) {
  ...
  async getNotifications(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      res.status(200).json(ApiResponse.success('Notifications fetched successfully', notifications));
    } catch (error) {
      if (error instanceof AppError) {
        res.status(error.statusCode).json(ApiResponse.error(error.message, error.statusCode));
        return;
  ...
  async markNotificationRead(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const notification = await activityService.markNotificationRead(req.user!, id);
