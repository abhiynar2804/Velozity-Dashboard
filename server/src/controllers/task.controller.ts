...
export class TaskController {
  async listTasks(
    ...
    try {
      const { projectId, status, priority, from, to } = req.query as {
        projectId?: string;
        status?: TaskStatus;
  ...
  async createTask(
    ...
    try {
      ...
      res
        .status(201)
        .json(ApiResponse.success("Task created successfully", task, 201));
    } catch (error) {
      if (error instanceof AppError) {
        res
          .status(error.statusCode)
  ...
  async deleteTask(
    ...
    try {
      ...
      res
        .status(200)
        .json(ApiResponse.success("Task deleted successfully", null));
    } catch (error) {
      if (error instanceof AppError) {
        res
          .status(error.statusCode)
