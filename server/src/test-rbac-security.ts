...
const createTestUser = async (name: string, email: string, role: UserRole) => {
...
async function runSecurityTests() {
  ...
  try {
    ...
    // ----------------------------------------------------
    console.log(
      "🔧 Setting up test entities (Admin, PM1, PM2, Dev1, Dev2, Client, Projects, Tasks)...",
    );
    ...
    const pm1 = await createTestUser(
      "Project Manager 1",
      `pm1.${timestamp}@test.com`,
      UserRole.PROJECT_MANAGER,
    );
    const pm2 = await createTestUser(
      "Project Manager 2",
      `pm2.${timestamp}@test.com`,
      UserRole.PROJECT_MANAGER,
    );
    const dev1 = await createTestUser(
      "Developer 1",
      `dev1.${timestamp}@test.com`,
      UserRole.DEVELOPER,
    );
    const dev2 = await createTestUser(
      "Developer 2",
      `dev2.${timestamp}@test.com`,
      UserRole.DEVELOPER,
    );
    adminId = admin.id;
    ...
    if (
      escalationRes.status !== 201 ||
      escalationData.data.user.role !== UserRole.DEVELOPER
    ) {
      throw new Error(
        `Public registration accepted an elevated role: ${JSON.stringify(escalationData)}`,
      );
    }
    await prisma.refreshToken.deleteMany({
      where: { userId: escalationData.data.user.id },
    });
    await prisma.user.delete({ where: { id: escalationData.data.user.id } });
    ...
    console.log(
      "   ✅ Test entities successfully provisioned in PostgreSQL database.\n",
    );
    ...
    if (unauthReq.status === 401) {
      ...
      throw new Error(
        `Expected 401 for unauthenticated request, got ${unauthReq.status}`,
      );
    }
    ...
    if (forgedRes.status === 401) {
      console.log(
        "   ✅ PASS: Tampered token with forged role rejected by JWT verification (401 Unauthorized)",
      );
    } else {
    ...
    if (devListUsersRes.status === 403) {
      console.log(
        "   ✅ PASS: Developer cannot view user directory (403 Forbidden)",
      );
    } else {
      throw new Error(
        `Expected 403 for Dev listing users, got ${devListUsersRes.status}`,
      );
    }
    ...
    if (devCreateClientRes.status === 403) {
      console.log(
        "   ✅ PASS: Developer cannot create clients (403 Forbidden)",
      );
    } else {
      throw new Error(
        `Expected 403 for Dev creating client, got ${devCreateClientRes.status}`,
      );
    }
    ...
    if (devCreateProjectRes.status === 403) {
      console.log(
        "   ✅ PASS: Developer cannot create projects (403 Forbidden)",
      );
    } else {
      throw new Error(
        `Expected 403 for Dev creating project, got ${devCreateProjectRes.status}`,
      );
    }

    // Attempt 2: Dev1 tries to directly access PM2 project where Dev1 has NO tasks
    const devAccessPM2ProjectRes = await fetch(
      `${baseUrl}/api/projects/${pm2ProjectId}`,
      {
        headers: { Authorization: `Bearer ${dev1Token}` },
      },
    );
    if (devAccessPM2ProjectRes.status === 403) {
      console.log(
        "   ✅ PASS: Developer cannot view unassigned project (403 Forbidden)",
      );
    } else {
      throw new Error(
        `Expected 403 for Dev accessing unassigned project, got ${devAccessPM2ProjectRes.status}`,
      );
    }
    ...
    const pm1ViewPM2Res = await fetch(
      `${baseUrl}/api/projects/${pm2ProjectId}`,
      {
    ...
    if (pm1ViewPM2Res.status === 403) {
      ...
      throw new Error(
        `Expected 403 for PM1 viewing PM2 project, got ${pm1ViewPM2Res.status}`,
      );
    }

    // Attempt 2: PM1 tries to update PM2's project
    const pm1UpdatePM2Res = await fetch(
      `${baseUrl}/api/projects/${pm2ProjectId}`,
      {
        method: "PATCH",
        headers: {
    ...
    if (pm1UpdatePM2Res.status === 403) {
      console.log(
        "   ✅ PASS: PM1 cannot update PM2's project (403 Forbidden)",
      );
    } else {
      throw new Error(
        `Expected 403 for PM1 updating PM2 project, got ${pm1UpdatePM2Res.status}`,
      );
    }
    ...
    if (pm1CreateTaskPM2Res.status === 403) {
      console.log(
        "   ✅ PASS: PM1 cannot create tasks in PM2's project (403 Forbidden)",
      );
    } else {
      throw new Error(
        `Expected 403 for PM1 creating task in PM2 project, got ${pm1CreateTaskPM2Res.status}`,
      );
    }
    ...
    const dev1ViewDev2TaskRes = await fetch(
      `${baseUrl}/api/tasks/${dev2TaskId}`,
      {
        headers: { Authorization: `Bearer ${dev1Token}` },
      },
    );
    if (dev1ViewDev2TaskRes.status === 403) {
      console.log(
        "   ✅ PASS: Developer 1 cannot view Developer 2's task (403 Forbidden)",
      );
    } else {
      throw new Error(
        `Expected 403 for Dev1 viewing Dev2 task, got ${dev1ViewDev2TaskRes.status}`,
      );
    }

    // Attempt 2: Dev1 tries to update Dev2's task status
    const dev1UpdateDev2TaskRes = await fetch(
      `${baseUrl}/api/tasks/${dev2TaskId}`,
      {
        method: "PATCH",
        headers: {
    ...
    if (dev1UpdateDev2TaskRes.status === 403) {
      console.log(
        "   ✅ PASS: Developer 1 cannot update Developer 2's task (403 Forbidden)",
      );
    } else {
      throw new Error(
        `Expected 403 for Dev1 updating Dev2 task, got ${dev1UpdateDev2TaskRes.status}`,
      );
    }
    ...
    const dev1TamperTaskRes = await fetch(
      `${baseUrl}/api/tasks/${dev1TaskId}`,
      {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
    ...
    if (dev1TamperTaskRes.status === 403) {
      console.log(
        "   ✅ PASS: Developer cannot modify non-status fields (403 Forbidden)",
      );
    } else {
      throw new Error(
        `Expected 403 for Dev editing title/reassigning, got ${dev1TamperTaskRes.status}`,
      );
    }
    ...
    const dev1UpdateStatusRes = await fetch(
      `${baseUrl}/api/tasks/${dev1TaskId}`,
      {
        method: "PATCH",
        headers: {
    ...
    if (
      dev1UpdateStatusRes.status === 200 &&
      dev1UpdateStatusData.data.status === "IN_PROGRESS"
    ) {
      console.log(
        "   ✅ PASS: Developer 1 successfully updated assigned task status to IN_PROGRESS (200)",
      );
    } else {
      throw new Error(
        `Status update failed: ${JSON.stringify(dev1UpdateStatusData)}`,
      );
    }

    // Check activity log was created
    const activityRes = await fetch(
      `${baseUrl}/api/activities?projectId=${pm1ProjectId}`,
      {
        headers: { Authorization: `Bearer ${pm1Token}` },
      },
    );
    ...
    if (
      ...
      console.log(
        "   ✅ PASS: Status transition activity recorded in database",
      );
    } else {
    ...
    if (adminProjectsRes.status === 200 && adminProjectsData.data.length >= 2) {
      console.log(
        `   ✅ PASS: Admin successfully viewed all ${adminProjectsData.data.length} projects across all PMs`,
      );
    } else {
    ...
    if (adminUsersRes.status === 200 && adminUsersData.data.length >= 5) {
      console.log(
        `   ✅ PASS: Admin successfully managed user registry (${adminUsersData.data.length} users)`,
      );
    } else {
